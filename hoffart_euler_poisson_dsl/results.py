...
def build_record(
    *,
    ...
    scale = 2.0 * math.pi / rhobar
    normalization = "gamma_paper = gamma_raw_sim * 2pi/rhobar (rhobar=%g, factor=%.6f); fit sur temps sim MAPPE" % (
        rhobar, scale)
    lo, hi = fit_window
...
def _selftest():
    ...
    assert paper_to_sim_time_window((0.40, 0.70), 1.0) == (0.40 * 2 * math.pi, 0.70 * 2 * math.pi)
    assert abs(gamma_to_paper_units(0.10, 1.0) - 0.10 * 2 * math.pi) < 1e-12
    assert gamma_to_paper_units(None) is None and gamma_to_paper_units(float("nan")) is None
    ...
    rec = build_record(
        engine="system-schur", mode=4, gamma_raw_sim=0.145, gamma_paper=0.911,
        fit_window=(0.60, 0.75), n=384, dt=1e-3, splitting="Strang", schur_theta=0.5,
        backend="kokkos-serial", rhobar=1.0, mpi_size=1,
        adc_cpp_sha_value="abc1234", adc_cases_sha_value="def5678",
    )
    assert rec["engine"] == "full-system-schur"
    assert rec["gamma_raw_sim"] == 0.145
    assert abs(rec["gamma_paper_units"] - 0.145 * 2 * math.pi) < 1e-12  # le 2pi EST applique au full
    assert "2pi/rhobar" in rec["normalization"]
    assert abs(rec["err_pct"] - 100.0 * (0.145 * 2 * math.pi - 0.911) / 0.911) < 1e-9
    assert rec["fit_window_paper"] == "0.6,0.75"
    assert rec["fit_window_sim"] == "%g,%g" % (0.60 * 2 * math.pi, 0.75 * 2 * math.pi)
    ...
    pend = build_record(
        engine="system-schur", mode=3, gamma_raw_sim=float("nan"), gamma_paper=0.772,
        fit_window=(0.40, 0.70), n=512, dt=1e-3, splitting="Strang", schur_theta=0.5,
    ...
    assert rec["gamma_paper_units"] != rec["gamma_raw_sim"]  # le facteur n'est pas 1

    print("OK results.py: fenetres verbatim+clock, labels moteur, mapping T3, record raw+paper, PENDING, IO")
