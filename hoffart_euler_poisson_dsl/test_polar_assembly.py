...
def _install_fake_adc():
    ...
    class FakeSystem:
        def __init__(self, **kwargs):
            ...
            self._nr = None
            self._nth = None
            self._state = None       # etat conservatif (3, ntheta, nr) le plus recemment pose
        ...
        def set_state(self, name, flat, *a, **k):
            self._state = np.asarray(flat, dtype=np.float64).reshape(3, self._nth, self._nr).copy()
...
def test_assembly_call_order_and_routing():
    _install_fake_adc()
...
def test_add_equation_uses_weno5_ssprk3_schur():
    _install_fake_adc()
    rp = _import_run_polar()
    ...
    assert src[0] == "CondensedSchur" and src[1].get("kind") == "electrostatic_lorentz"
    print("OK (3) WENO5 + Rusanov + SSPRK3 + CondensedSchur(electrostatic_lorentz)")


def test_strang_switches_split_factory():
    _install_fake_adc()
    rp = _import_run_polar()
    sim = rp.build_polar_system(24, 16, 4, _params(), _Args(strang=True))
    eq = [c for c in sim.calls if c[0] == "add_equation"][0]
    assert eq[2]["time"][0] == "Strang", "--strang doit utiliser adc.Strang (2e ordre)"
...
def test_density_is_annular_tophat():
    ...
    assert np.allclose(rho[:, ~inside], params.rho_min), "fond rho_min hors anneau"
...
def test_state_radial_velocity_is_exb_drift():
    ...
    U = np.asarray(state_flat).reshape(3, args.ntheta, args.nr)  # (rho, mom_r, mom_theta)
    rho, mom_r = U[0], U[1]
    ...
    v_r = mom_r / rho
    assert np.allclose(v_r, -gt / params.omega, atol=1e-12), "v_r doit rester ExB -grad_theta/B"
...
def test_state_azimuthal_velocity_solves_radial_balance():
    ...
    assert np.max(np.abs(residual) / scale) < 1e-10, \
        "v_theta doit annuler la quadratique de bilan radial (residu rel %.2e)" % np.max(np.abs(residual) / scale)
    ...
    assert np.allclose(v_th, forcing_over_B, rtol=1e-6, atol=1e-300), \
        "a B grand v_theta doit valoir forcing/B = (d_r phi + cs2 d_r rho/rho)/B"
    print("OK (5b) v_theta = racine du bilan radial (equilibre rotatif), -> forcing/B a B grand")
...
def test_equilibrium_v_theta_reduces_to_exb_when_cold():
    ...
    assert np.allclose(v_th, exb, rtol=1e-6, atol=1e-300), \
        "cs2=0 : v_theta doit se reduire a la derive ExB grad_r/B (residu %.2e)" % np.max(np.abs(v_th - exb))
...
def test_check_equilibrium_is_stationary():
    ...
    rp = _import_run_polar()
    params = _params()
    args = _Args(cs2=1.0e-4, dt=1.0e-3, cfl=0.0, max_steps_check=20,
                 check_modes=[1, 2, 3, 4, 5], check_tol=0.05)
    ok, report = rp.check_equilibrium(params, args)
    assert all(row["finite"] for row in report), "potentiel doit rester fini (pas de NaN)"
    ...
    print("OK (5c) check_equilibrium : anneau d'equilibre stationnaire (chaque mode plat)")


def test_compute_frozen_residual_captures_scheme_drift():
    ...
    probe = adc.System(mesh=adc.PolarMesh(args.r_min, params.radius, args.nr, args.ntheta))
    probe._nr, probe._nth = args.nr, args.ntheta
    drift = probe._spurious_drift()
    assert np.max(np.abs(R_eq - drift)) < 1e-14, "R_eq doit capturer la derive parasite step(U_eq)-U_eq"
    assert np.max(np.abs(R_eq)) > 1e-6, "R_eq doit etre O(1e-3) non trivial (schema non bien pose)"
    print("OK (c1) compute_frozen_residual capture la derive du schema (R_eq = step(U_eq) - U_eq)")
...
def test_step_frozen_subtracted_call_order_and_cancellation():
    ...
    U = sim.get_state("ne")
    assert np.max(np.abs(U - U_eq)) < 1e-12, "step()-R_eq doit rendre U_eq invariant (point fixe)"
    print("OK (c2) step_frozen_subtracted : ordre facade correct + U_eq point fixe exact")
...
def test_run_mode_frozen_uses_subtracted_stepping():
    ...
    assert np.all(np.isfinite(result["amplitudes"])), "amplitudes finies (option c stabilise)"
    ...
    print("OK (c4) run_mode frozen : carte step()-R_eq cablee, step_cfl jamais appele")
...
def test_main_frozen_ignores_cfl_and_runs_quick():
    ...
    print("OK (c5) main --quick --cfl ignore sous frozen-equilibrium, run complet")
...
def test_multirank_is_rejected():
    ...
    try:
        ...
        assert raised, "un run multi-rang doit etre refuse (etage Schur polaire = boite unique)"
    finally:
        sys.argv = argv
        os.environ.clear()
        os.environ.update(saved)
