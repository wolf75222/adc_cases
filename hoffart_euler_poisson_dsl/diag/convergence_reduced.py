...
TWO_PI = 2 * math.pi
PW = {3: (0.40, 0.70), 4: (0.60, 0.75), 5: (1.15, 1.35)}
PAP = {3: 0.772, 4: 0.911, 5: 0.683}


def run(l: int, n: int, cfl: float = 0.4, t_end: float | None = None) -> float:
    """Run ExB normalise cartesien au mode l ; renvoie gamma_paper (gamma_raw*2pi)."""
    ...
    r = np.hypot(X, Y)
    th = np.arctan2(Y, X)
    d = RHO_MAX * (1 - DELTA + DELTA * np.sin(l * th))
    s.set_density("ne", np.where((r >= R0) & (r <= R1), d, RHO_MIN).reshape(-1))
    thc = np.linspace(0, TWO_PI, 1024, endpoint=False)
    xs = RW + R0 * np.cos(thc)
    ys = RW + R0 * np.sin(thc)
    fi = xs / h - 0.5
    fj = ys / h - 0.5
    ...
    while True:
        t = float(s.time())
        phi = np.asarray(s.potential(), float).reshape(n, n)
        ...
        ts.append(t)
        cs.append(abs((np.fft.rfft(v) / v.size)[l]))
        if t >= t_end:
            break
        s.step_cfl(cfl)
    ts = np.array(ts)
    cs = np.array(cs)
    lo, hi = PW[l]
    m = (ts >= lo * TWO_PI) & (ts <= hi * TWO_PI) & (cs > 0)
    g = (
        float(np.polyfit(ts[m], np.log(cs[m]), 1)[0])
        if m.sum() > 4
        else float("nan")
    )
    return g * TWO_PI
...
for n in (64, 96, 128, 192, 256):
    ...
    for l in (3, 4, 5):
        gp = run(l, n)
        row.append("%.3f(%+.1f%%)" % (gp, 100 * (gp - PAP[l]) / PAP[l]))
