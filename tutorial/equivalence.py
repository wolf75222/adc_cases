...
def _final_density(sim, n_steps):
    for _ in range(n_steps):
        sim.step_cfl(0.4)
    return np.asarray(sim.density("ne"))


def _build_and_run(add_block_fn, model_or_compiled, ne0, n_steps):
    sim = make_system(ne0)
...
def main():
    ...
    d_helper = _build_and_run(add_bricks_block, models.diocotron(B0=B0, alpha=ALPHA, n_i0=n_i0),
                              ne0, n_steps)
    d_bricks = _build_and_run(add_bricks_block, diocotron_from_bricks(n_i0), ne0, n_steps)
    ...
    print("backend DSL %r : helper==briques %s, briques==DSL %s "
          "(max|b-DSL| = %.3e)" % (backend, eq_hb, eq_bd, float(np.max(np.abs(d_bricks - d_dsl)))))
    assert eq_hb, "helper models.diocotron != composition de briques"
    assert eq_bd, "modele DSL != briques (une formule diverge des conventions du coeur)"
    print("OK tutorial/equivalence (diocotron helper == briques == DSL, bit-identique)")
