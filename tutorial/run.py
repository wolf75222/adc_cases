...
from adc_cases import (
    models,
)  # noqa: E402  (helper specialise : l'oracle de reference)
...
def diocotron_from_dsl(n_i0: float) -> dsl.Model:
    ...
    (n,) = m.conservative_vars(
        "n"
    )  # 1 variable conservative, role canonique Density
    m.aux("phi")  # potentiel (contrat aux ; non lu par le flux)
    grad_x = m.aux(
        "grad_x"
    )  # composantes du gradient de phi, fournies par le solveur
    grad_y = m.aux("grad_y")
    ...
    m.eigenvalues(
        x=[vx], y=[vy]
    )  # 1 onde : la vitesse de derive (borne Rusanov / CFL)
    ...
    for cand in ("production", "aot"):
        try:
            compiled = model.compile(
                os.path.join(so_dir, "tutorial_%s.so" % cand),
                include,
                backend=cand,
            )
...
def add_bricks_block(sim: adc.System, model: adc.Model) -> None:
    """Bloc natif : add_block prend un ModelSpec (briques), schema minmod + Rusanov, SSPRK2."""
    sim.add_block(
        "ne", model=model, spatial=adc.Spatial(minmod=True), time=adc.Explicit()
    )
...
def make_figures(
    ...
    times: list,
    amps: list,
    final_bricks: np.ndarray,
    final_dsl: np.ndarray,
    vmax: float,
) -> None:
    import matplotlib
    ...
    axc.set_xlabel("temps")
    axc.set_ylabel("amplitude L2 de la perturbation")
    ...
    axm.set_title("densite finale $n_e$")
    axm.set_xticks([])
    axm.set_yticks([])
    fig.colorbar(im, ax=axm, fraction=0.046)
    fig.tight_layout()
    fig.savefig(os.path.join(FIGDIR, "tutorial_growth.png"), dpi=120)
    plt.close(fig)
    ...
    anim = animation.FuncAnimation(
        figg, upd, frames=len(frames), interval=120, blit=False
    )
    anim.save(
        os.path.join(FIGDIR, "tutorial.gif"),
    ...
    a0.set_title("briques natives")
    a1.set_title("formules (DSL)")
    for a in (a0, a1):
        a.set_xticks([])
        a.set_yticks([])
    figd.suptitle(
        "meme physique, deux fronts : max|briques - DSL| = %.0e"
        % float(np.max(np.abs(final_bricks - final_dsl)))
    )
    figd.tight_layout()
    figd.savefig(os.path.join(FIGDIR, "tutorial_bricks_vs_dsl.png"), dpi=120)
    plt.close(figd)
...
def main() -> None:
    ...
    n_i0 = float(
        ne0.mean()
    )  # fond neutralisant = moyenne (solubilite de Poisson periodique)

    print(
        "=== tutorial : diocotron en helper / briques / formules (meme physique) ==="
    )
    print(
        "grille %d x %d, %d pas, CFL 0.4, n_i0 = %.6e" % (n, n, n_steps, n_i0)
    )
    ...
    assert (
        eq_hb
    ), "le helper models.diocotron diverge de la composition de briques (pourtant identiques)"
    assert (
        eq_bd
    ), "le modele DSL n'est pas bit-identique aux briques (une formule diverge du coeur)"
    ...
    print(
        "OK tutorial (3 fronts equivalents, backend %r, figures dans figures/)"
        % backend
    )
