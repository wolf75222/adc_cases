...
import realizability as realz  # noqa: E402
import relaxation as relax  # noqa: E402
...
def check_oracle_parity():
    ...
    _, s_vec = realz.standardized_moments(field)
    lmin, lmid, lmax = realz._p2p2_eigs(s_vec)
    ny, nx = field.shape[1:]
    ds = dl = 0.0
    for a in range(ny):
        for b in range(nx):
            _, so = relax.m2cs4(field[:, a, b])
            ds = max(ds, max(abs(so[pq] - s_vec[pq][a, b]) for pq in so))
            p = relax.p2p2_2d(so[(0, 3)], so[(0, 4)], so[(1, 1)], so[(1, 2)], so[(1, 3)],
...
def check_realizable_and_not():
    good, real = realz.field_realizability(_realizable_field())
    assert np.all(real["all"]), "a Maxwellian field must be fully realizable"
    assert realz.summarize(good, real)["frac_nonrealizable"] == 0.0
    bad = _realizable_field(seed=1)
    bad[0, 0, 0] = -1.0  # negative density in one cell
    _, realbad = realz.field_realizability(bad)
...
def check_symmetry():
    ...
    uni = sym.uniformity_residual(_field_with_m00(np.ones((n, n))))
    grad = sym.uniformity_residual(_field_with_m00(1.0 + 0.5 * x * np.ones((n, n))))
    ...
    res_y = sym.axis_uniformity_residual(_field_with_m00(m_y), axis=0)
    broke_y = sym.axis_uniformity_residual(_field_with_m00(m_y + 0.1 * y), axis=0)
    ...
    r2 = x ** 2 + y ** 2
    rot = sym.rotational_residual(_field_with_m00(np.exp(-20 * r2)))
    asym = sym.rotational_residual(_field_with_m00(np.exp(-20 * r2) + 0.3 * x * np.ones((n, n))))
    ...
    pure = sym.mode_purity_residual(_field_with_m00(clean * np.ones((n, n))), kx, ky)
    noisy = sym.mode_purity_residual(_field_with_m00(clean + 0.1 * np.sin(2 * np.pi * x)), kx, ky)
...
def check_recovery():
    ...
    res = {r["name"]: r for r in realz.evaluate(seq)}
    pos = res["positivity"]
    ...
    strict = realz.RealizabilityCheck("positivity_strict", realz._ok_positivity, must_recover=False)
    sres = realz.evaluate(seq, [strict])[0]
