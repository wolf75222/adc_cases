...
def check_crossing_relax_golden():
    """(4) golden spatial AVEC relaxation active : enchainement transport x relaxation du pilote
    de production (main_pb_2Dcrossing_2DHyQMOM15.m : flagrelax = 1, Ma = 20). golden_relax couvre
    la projection ISOLEE et golden_hll tourne flagrelax = 0 ET Ma = 2 (regime ou relaxation15 ne
    se declenche jamais) : ce golden est le SEUL a enchainer transport puis relaxation15 par
    cellule a chaque pas, comme le pilote.
    ...
    g = os.path.join(HERE, "golden")
    meta = np.atleast_1d(np.loadtxt(os.path.join(g, "golden_crossing_relax_meta.csv"),
                                    delimiter=","))
    n, ma, nsteps = int(meta[0]), float(meta[1]), int(meta[2])
    lamin = float(meta[4])
    dts = np.atleast_1d(np.loadtxt(os.path.join(g, "golden_crossing_relax_dts.csv"),
                                   delimiter=","))
    rawin = np.loadtxt(os.path.join(g, "golden_crossing_relax_in.csv"), delimiter=",")
    rawout = np.loadtxt(os.path.join(g, "golden_crossing_relax_out.csv"), delimiter=",")
    U0 = np.stack([rawin[k * n:(k + 1) * n, :].T for k in range(15)], axis=0)   # (15, ny, nx)
    gold = np.stack([rawout[k * n:(k + 1) * n, :].T for k in range(15)], axis=0)
    ...
    so = m.compile(os.path.join(case_output_dir("hyqmom15"), "hyqmom15_crossing_relax.so"),
                   adc_include(), backend="production")
    fn = make_corner_eigs()

    def run(project):
        sim = adc.System(n=n, L=1.0, periodic=True)
        sim.add_equation("mom", model=so,
                         spatial=adc.FiniteVolume(limiter="none", riemann="hll"),
        ...
        with np.errstate(all="ignore"):
            for dt in dts:
                ...
                if project:
                    sim.set_state("mom", relax_field(np.array(sim.get_state("mom")),
                                                     lamin, ma, corner_eigs=fn))
        return np.array(sim.get_state("mom"))

    def l2(a):
        return float(np.sqrt(np.sum((a - gold) ** 2)) / np.sqrt(np.sum(gold ** 2)))

    U = run(project=True)
    assert np.all(np.isfinite(U)), "replay transport x relaxation : etat non fini"
    gap = l2(U)
    assert gap < 5e-8, ("fidelite transport x relaxation : ecart L2 relatif %.2e au golden "
    ...
    Uraw = run(project=False)
    gap_raw = l2(Uraw)
    assert gap_raw > 1e2 * gap, ("la relaxation n'est pas materiellement active dans le golden : "
                                 "replay nu a %.2e vs %.2e avec projection (contraste insuffisant)"
                                 % (gap_raw, gap))
    print("(4) golden transport x relaxation (flagrelax=1, Ma=%g, %d pas) : replay euler+relax "
