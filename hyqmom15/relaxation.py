...
def m2cs4(M4):
    ...
    for (p, q) in MOMENT_PQ:
        ...
        for i in range(p + 1):
            for j in range(q + 1):
                acc += (_binom(p, i) * _binom(q, j) * (-u) ** (p - i) * (-v) ** (q - j)
                        * mn[(i, j)])
        C[(p, q)] = acc
    sx, sy = np.sqrt(C[(2, 0)]), np.sqrt(C[(0, 2)])
    S = {pq: C[pq] / (sx ** pq[0] * sy ** pq[1]) for pq in MOMENT_PQ if pq[0] + pq[1] >= 2}
    return C, S


def cs4_to_m4(m00, u, v, C):
    ...
    for k, (p, q) in enumerate(MOMENT_PQ):
        ...
        for i in range(p + 1):
            for j in range(q + 1):
                cij = 1.0 if (i, j) == (0, 0) else (0.0 if (i, j) in ((1, 0), (0, 1))
                                                    else C[(i, j)])
                if cij == 0.0:
                    continue
                acc += _binom(p, i) * _binom(q, j) * u ** (p - i) * v ** (q - j) * cij
        out[k] = m00 * acc
    return out
...
def p2p2_2d(a03, a04, a11, a12, a13, a21, a22, a30, a31, a40):
    t2 = a03 * a12
    t3 = a03 * a21
    t4 = a12 * a21
    t5 = a12 * a30
    ...
    t39 = t32 * t35
    t37 = -t36
    flat = [t32 * (-a40 + t10 + t24 - a11 * t6 * 2.0 + a40 * t7 + a30 ** 2 + 1.0),
            t39, t37, t39,
...
def collision15_anisotropic(s03, s04, s11, s12, s13, s21, s22, s30, s31, s40, lamin):
    small = 1.0e-6
    S = np.array([s03, s04, s11, s12, s13, s21, s22, s30, s31, s40])

    lam = np.sort(np.real(np.linalg.eigvals(
    ...
    if H20 < small or H02 < small:
        S11 = np.sign(s11)
        return np.array([S3 * S11, S4, S11, S3, S4 * S11, S3 * S11, S4, S3, S4 * S11, S4])
    ...
    s22_1 = ((3.0 * S11) / 8.0 - S04 / 32.0 - S13 / 8.0 - S31 / 8.0 - S40 / 32.0
             + (3.0 * S03 * S12) / 32.0 - (S04 * S11) / 32.0 + (3.0 * S03 * S21) / 32.0
             - (S11 * S13) / 8.0 + (S03 * S30) / 32.0 + (9.0 * S12 * S21) / 32.0
             - (S11 * S31) / 8.0 + (3.0 * S12 * S30) / 32.0 - (S11 * S40) / 32.0
             + (3.0 * S21 * S30) / 32.0 + S03 ** 2 / 64.0 + (3.0 * S11 ** 2) / 8.0
             + S11 ** 3 / 8.0 + (9.0 * S12 ** 2) / 64.0 + (9.0 * S21 ** 2) / 64.0
             + S30 ** 2 / 64.0 + 1.0 / 8.0) / ((3.0 * S11) / 16.0 + 3.0 / 16.0)
    s22_2 = -((S13 / 8.0 - (3.0 * S11) / 8.0 - S04 / 32.0 + S31 / 8.0 - S40 / 32.0
               - (3.0 * S03 * S12) / 32.0 + (S04 * S11) / 32.0 + (3.0 * S03 * S21) / 32.0
               - (S11 * S13) / 8.0 - (S03 * S30) / 32.0 - (9.0 * S12 * S21) / 32.0
               - (S11 * S31) / 8.0 + (3.0 * S12 * S30) / 32.0 + (S11 * S40) / 32.0
               - (3.0 * S21 * S30) / 32.0 + S03 ** 2 / 64.0 + (3.0 * S11 ** 2) / 8.0
...
def relax15(M4, lamin, Ma, corner_eigs=None):
    ...
    if s11 >= 1.0 - small:
        ...
        if H2 <= 0.0:
            S4 = S3 ** 2 + 1.0 + small
        s03 = S3; s21 = S3; s12 = S3; s30 = S3
        s04 = S4; s13 = S4; s22 = S4; s31 = S4; s40 = S4
    elif s11 <= -1.0 + small:
        s11 = -1.0 + small
        flagS11 = 1
        S3 = np.sqrt(abs(s03 * s30)) * np.sign(s30)
    ...
    # blocs est complexe ».
    sd = {"s03": s03, "s04": s04, "s11": s11, "s12": s12, "s13": s13,
          "s21": s21, "s22": s22, "s30": s30, "s31": s31, "s40": s40}
    lamx, lamy = (corner_eigs if corner_eigs is not None else make_corner_eigs())(sd)
    tol = 1e-9
    ...
    if flagS11 != 1:
        out = collision15_anisotropic(s03, s04, s11, s12, s13, s21, s22, s30, s31, s40,
                                      lamin)
    ...
    Cn = {(2, 0): c20, (1, 1): S11 * sx * sy, (0, 2): c02,
          (3, 0): S30 * sx ** 3, (2, 1): S21 * sx ** 2 * sy,
          (1, 2): S12 * sx * sy ** 2, (0, 3): S03 * sy ** 3,
          (4, 0): S40 * sx ** 4, (3, 1): S31 * sx ** 3 * sy,
          (2, 2): S22 * sx ** 2 * sy ** 2, (1, 3): S13 * sx * sy ** 3,
          (0, 4): S04 * sy ** 4}
...
def _standardized_state(sd):
    """Moments bruts de l'etat standardise (M00 = 1, u = v = 0, sigma = 1) : M_pq = s_pq."""
    vals = {"M00": 1.0, "M10": 0.0, "M01": 0.0, "M20": 1.0, "M02": 1.0,
            "M11": sd["s11"], "M30": sd["s30"], "M40": sd["s40"], "M21": sd["s21"],
            "M31": sd["s31"], "M12": sd["s12"], "M22": sd["s22"], "M03": sd["s03"],
            "M13": sd["s13"], "M04": sd["s04"]}
...
def make_corner_eigs():
    ...
    m = build_moment_model("hyqmom15_relax_jac", robust=False, exact_speeds=True)
    IX = [12, 13, 14]
    IY = [3, 8, 4]
    ...
    def fn(sd):
        ...
        env = m._m._env(U, {})  # cons + primitives derivees (les exprs gardent des symboles)
        bx = np.array([[_ev(EX[a][b], env) for b in range(3)] for a in range(3)])
        by = np.array([[_ev(EY[a][b], env) for b in range(3)] for a in range(3)])
...
def relax_field(U, lamin, Ma, corner_eigs=None):
