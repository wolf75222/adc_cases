...
def poisson_fft_ref(
    ...
    mask = K2 != 0
    phi_hat[mask] = -rho_hat[mask] / (
        lam**2 * K2[mask]
    )  # signe electron (poisson_fft.m)
...
def diocotron_state(n: int, ic_matlab_bug: bool = False) -> np.ndarray:
    ...
    X, Y = np.meshgrid(
        xm, xm, indexing="xy"
    )  # X varie sur l'axe 1 (colonnes) = x
    R = np.sqrt(X**2 + Y**2)
...
def build_sim(
    ...
    m = build_moment_model(
        name="hyqmom15_vp" + ("_ex" if exact_speeds else ""),
...
K_MIN = (
    np.sqrt(2.0) * np.pi / 1.0
)  # k_min = sqrt(2)*pi sur le domaine de cote 1
...
def matlab_dt_source(n: int, cfl: float = 0.4) -> dict:
    ...
    probe = build_sim(
        n, rho_bg=float(U0[0].mean()), riemann="hll", exact_speeds=True
    )
...
def check_diocotron_dt_source_matlab() -> None:
    ...
    assert dt_src < dt_cfl0, (
        "borne source non mordante : dt_source=%.3e >= dt_cfl=%.3e -- le "
    ...
    for _ in range(nsteps):
        sim.step(
            min(dt_cfl0, dt_src)
        )  # min(dt, dt_source) : dt_source mord ici
    ...
    assert drift < 1e-12, (
        "dt source MATLAB : masse non conservee (%.2e)" % drift
    )
    print(
        "(8) borne dt source MATLAB (opt-in) : lambda_flux=%.3f, max_speed=%.3f, "
...
def check_poisson_oracle() -> None:
    ...
    phi_ref = (
        -(eps / (DEBYE**2 * k**2)) * np.cos(k * xm)[None, :] * np.ones((n, n))
    )
    phi0 = phi - phi.mean()
    err = float(np.max(np.abs(phi0 - phi_ref)) / np.max(np.abs(phi_ref)))
    assert err < 2e-2, (
        "Poisson : phi != analytique (err rel %.3e) -- signe ou echelle" % err
    )
    print(
        "(1) oracle Poisson : phi == -eps cos(kx)/(lam^2 k^2) a %.1e pres (n = %d) -- OK"
        % (err, n)
    )
    ...
    free = m_free.compile(
        os.path.join(case_output_dir("hyqmom15"), "hyqmom15_free.so"),
        adc_include(),
        backend="aot",
    )
    ...
    assert d < 1e-10, (
        "source compilee : Ex implicite != -grad_x(phi) centre (err rel %.3e ; "
    ...
    print(
        "(2) source compilee : Ex implicite == -grad phi centre (%.1e) == analytique (%.1e) "
        "-- OK" % (d, d2)
    )
...
def check_diocotron() -> None:
    ...
    U0 = diocotron_state(n)
    sim = build_sim(
        n, rho_bg=float(U0[0].mean())
    )  # fond neutralisant = moyenne de l'anneau
    sim.set_state("mom", U0)
    sim.solve_fields()
    mass0 = float(U0[0].sum())
    ...
    for s in range(nsteps):
        ...
        if (s + 1) % every == 0:
            snaps.append(
                sim.write(
                    os.path.join(out_dir, "diocotron"), format="npz", step=s + 1
                )
            )
    U = np.array(sim.get_state("mom"))
    ...
    print(
        "(3) diocotron : %d pas finis, M00 > 0, derive de masse %.1e, phi fini -- OK"
        % (nsteps, drift)
    )
    ...
    assert any(
        "state" in k or "mom" in k for k in last.files
    ), "snapshot sans etat"
    assert any("phi" in k for k in last.files), "snapshot sans phi"
    print(
        "(5) snapshots cadences : %d fichiers npz avec etat + phi -- OK"
        % len(snaps)
    )
...
def check_diocotron_hll_exact() -> None:
    ...
    drift = abs(float(U[0].sum()) - mass0) / mass0
    assert drift < 1e-12, (
        "diocotron HLL exact : masse non conservee (%.2e)" % drift
    )
    assert np.all(np.isfinite(np.array(sim.potential()))), "phi non fini"
    print(
        "(6) diocotron en riemann='hll' + vitesses exactes : %d pas stables, masse conservee "
...
def check_poisson_solvers() -> None:
    ...
    for solver in ("fft", "fft_spectral", "geometric_mg"):
        ...
        errs[solver] = float(
            np.max(np.abs(phis[solver] - phi_ref)) / np.max(np.abs(phi_ref))
        )
    assert errs["fft_spectral"] < 1e-11, (
        "fft_spectral devrait etre exact sur la sinusoide (err %.2e)"
        % errs["fft_spectral"]
    )
    for sname in ("fft", "geometric_mg"):
        assert (
            1e-5 < errs[sname] < 1e-2
        ), "%s : fenetre O(h^2) attendue (err %.2e)" % (sname, errs[sname])
    dmg = float(
        np.max(np.abs(phis["fft"] - phis["geometric_mg"]))
        / np.max(np.abs(phi_ref))
    )
    assert dmg < 1e-5, (
        "fft != geometric_mg (%.2e) : meme operateur attendu" % dmg
    )
    print(
        "(7) Poisson : fft_spectral == continu a %.1e ; fft == MG (%.1e) a O(h^2) du "
...
def measure_ic_impact(n: int = 64, nsteps: int = 40) -> None:
    ...
    def run(ic_matlab_bug):
        ...
        ke = float(
            np.sum(U[1] ** 2 + U[6] ** 2) / 2.0
        )  # 0.5 (M10^2 + M01^2), proxy energie cinetique
        return a0, a1, ke, U
    ...
    print(
        "=== impact orientation ExB (mode %d, n=%d, %d pas) ==="
        % (MODE, n, nsteps)
    )
    print(
        "  derive standard : A4(0)=%.4e -> A4(%d)=%.4e (gain x%.3f), KE=%.4e"
        % (a0_std, nsteps, a1_std, a1_std / a0_std, ke_std)
    )
    print(
        "  bug MATLAB      : A4(0)=%.4e -> A4(%d)=%.4e (gain x%.3f), KE=%.4e"
        % (a0_bug, nsteps, a1_bug, a1_bug / a0_bug, ke_bug)
    )
    print(
        "  ecart relatif A4 final = %.3e ; ecart KE = %.3e ; max|U_std - U_bug| = %.3e"
        % (
            abs(a1_std - a1_bug) / max(a1_std, 1e-300),
            abs(ke_std - ke_bug) / max(ke_std, 1e-300),
            du,
        )
    )
...
def main(argv: list[str] | None = None) -> None:
    ...
    ap = argparse.ArgumentParser(
        description="Cas hyqmom15 : Vlasov-Poisson 15 moments, anneau diocotron"
    )
    ap.add_argument(
        "--ic-matlab-bug",
        action="store_true",
    ...
    print(
        "=== hyqmom15/run_diocotron : Vlasov-Poisson 15 moments, anneau diocotron ==="
    )
