...
INCLUDE = os.environ.get("ADC_INCLUDE",
...
def chk(cond, label, detail=""):
    global fails
    print("  [%s] %s%s" % ("OK " if cond else "XX ", label, ("  " + detail) if detail else ""))
    if not cond:
        fails += 1
...
def _p2p2_lammin(m4):
    _, s = R.m2cs4(m4)
    p2 = R.p2p2_2d(s[(0, 3)], s[(0, 4)], s[(1, 1)], s[(1, 2)], s[(1, 3)], s[(2, 1)], s[(2, 2)],
                   s[(3, 0)], s[(3, 1)], s[(4, 0)])
    return float(np.sort(np.real(np.linalg.eigvals(p2)))[0])


def _compile_projector(ma, lamin, tmp, cxx):
    """Emit the projection brick for @p ma and compile a tiny main calling project(U, a) per cell."""
    m = MOD.build_moment_model("brick_ma%d" % int(ma), robust=False, exact_speeds=True,
                               projection=True, Ma=ma, lamin=lamin)
    ...
    cp = subprocess.run([cxx, "-std=c++17", "-O2", "-I", INCLUDE, main, "-o", exe],
...
def main():
    ...
    if not cxx or not os.path.isdir(INCLUDE):
        ...
        return 0
    inm = np.loadtxt(os.path.join(HERE, "golden", "golden_relax_in.csv"), delimiter=",")
    meta = np.loadtxt(os.path.join(HERE, "golden", "golden_relax_meta.csv"), delimiter=",")
    lamin0 = float(meta[0, 0])
    fn = R.make_corner_eigs()
    tmp = tempfile.mkdtemp()
    try:
        exe = {ma: _compile_projector(ma, lamin0, tmp, cxx) for ma in (2.0, 20.0)}
        ...
        for ma in (2.0, 20.0):
            ...
            for k, t in enumerate(idx):
                ...
                print("  state %2d br%d Ma=%2d  absdiff %.3e" % (t, int(meta[t, 2]), int(ma), d))
        chk(worst_a < 1e-10, "compiled project == relax15 (worst %.3e)" % worst_a)

        print("== (B) compiled field projection == relax_field on (15,ny,nx) ==")
        ny, nx = 4, 5
        ...
        def rate(a):
            return sum(1 for k in range(a.shape[1]) if _p2p2_lammin(a[:, k]) <= lamin0) / a.shape[1]

        before = rate(stress)
        nat = rate(_run(exe[20.0], stress, tmp))
        orc = rate(R.relax_field(stress.reshape(15, 1, -1), lamin0, 20.0,
                                 corner_eigs=fn).reshape(15, -1))
        print("  non-realizable rate before=%.3f native=%.3f oracle=%.3f" % (before, nat, orc))
        chk(abs(nat - orc) < 1e-9 and nat <= before + 1e-12, "native rate == oracle and not increased")

        print("== (D) no-regression : projection=False emits no project hook ==")
        m_np = MOD.build_moment_model("noproj", robust=False, exact_speeds=True, projection=False)
        src_np = m_np._m.emit_cpp_brick(name="NoProj")
        m_p = MOD.build_moment_model("withproj", robust=False, exact_speeds=True, projection=True,
                                     Ma=2.0, lamin=lamin0)
        src_p = m_p._m.emit_cpp_brick(name="WithProj")
        chk("State project" not in src_np, "projection=False -> no project hook")
        chk("State project" in src_p, "projection=True -> project hook present")
        chk("dense_eig.hpp" in src_p, "projector brick includes dense_eig.hpp (eig witness)")
    finally:
