...
| D8 | `time_step.m` dispatches `Euler`/`RK2`/`RK3` (`euler_step`/`rk2_step`/`rk3_step`); the RK stages freeze `phi` across substeps (no per-stage Poisson re-solve) | `matlab_ref/time_policy.py` maps `time_scheme` -> `adc.Explicit(method=euler/ssprk2/ssprk3)` (ADC-379); `System.step(dt)` solves the fields once per macro-step and keeps `phi` frozen through the RK stages, matching `time_step.m`. The five committed cases all declare `Euler` | Supported by the port layer, NOT deferred: each case's `time_scheme` is honoured. The `adc.integrate.ssprk*_step` Python helpers re-solve Poisson per stage, a different (more coupled) mode that is not strict Matlab parity | ADC-379 |
