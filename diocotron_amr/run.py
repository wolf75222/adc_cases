...
def main() -> None:
    ...
    print(
        "# n_base=%d regrid_every=10 band_mode=%d  n_i0=%.4f" % (N, MODE, n_i0)
    )
    print(
        "# %-5s %-8s %-8s %-13s %-11s"
    ...
    for k in range(NSTEPS):
        ...
        assert (
            npatch >= 2
        ), "raffinement insuffisant au pas %d : n_patches()=%d (< 2)" % (
            k,
            npatch,
        )
        assert drel < TOL_MASS, "masse non conservee au pas %d : drel=%.3e" % (
            k,
            drel,
        )
        ...
        if k % 10 == 0 or k == NSTEPS - 1:
            print(
                "  %-5d %-8.4f %-8d %-13.8e %-11.3e"
                % (k, sim.time(), npatch, mass, drel)
            )

    dens = sim.density()
    print("# patchs observes : %s" % sorted(patches_seen))
    print(
        "# masse : init=%.12e final=%.12e drel=%.3e"
        % (mass0, sim.mass(), relative_drift(sim.mass(), mass0))
    )
    print(
        "# densite : min=%.6e max=%.6e" % (float(dens.min()), float(dens.max()))
    )
    assert (
        min(patches_seen) >= 2
    ), "AMR n'a jamais couvert la bande (< 2 patchs)"
    ...
    print(
        "# controle (seuil %.0e) : patches=%d  ecart_sup solution=%.6e"
