...
def _build_sim(adc, sc, front, n, poisson, model, compiled):
    ...
    if front == "bricks":
        sim.add_block("gas", model=model, spatial=sc.spatial_bricks(), time=adc.Explicit())
    else:
...
# --------------------------------------------------------------------------------------------------
def run_front(front, n, steps, warmup, poisson, dsl_cache, state_out):
    t = time.perf_counter()
    ...
    if front == "dsl":
        ...
        if compiled is None:
            raise RuntimeError("aucun backend DSL n'a compile le modele Euler sur (%s)" % last)
    ...
    rec = prov_mod.provenance({
        ...
        "ranks": 1,
        "gpus": 0,
        "nx": n, "ny": n, "boxes": 1, "max_grid": n,
        "workload": sc.WORKLOAD, "limiter": sc.LIMITER, "flux": sc.FLUX, "recon": sc.RECON,
        "time": sc.TIMEINT,
        "poisson": poisson, "dt": dt, "warmup": warmup, "steps": steps,
        "dsl_backend": dsl_backend, "dsl_cache": dsl_cache,
        "stages": stages, "total_cold_user_s": total_cold,
        "hot_ms_per_step": {"median": med, "p10": p10, "p90": p90, "cv": cv},
        "advance_ms_per_step": advance_ms,
        "phases_ms_per_step": {"solve_fields": solve_fields_ms},
        "cells_per_s": cells_per_s,
        "invariants": {"mass": mass, "rho_min": rho_min, "p_min": p_min, "nan": has_nan},
    })
    if state_out:
...
# --------------------------------------------------------------------------------------------------
def _spawn_worker(front, n, steps, warmup, poisson, dsl_cache, json_out, state_out, cache_dir=None):
    ...
    cmd = [sys.executable, os.path.abspath(__file__), "--front", front, "--n", str(n),
           "--steps", str(steps), "--warmup", str(warmup), "--poisson", poisson,
           "--dsl-cache", dsl_cache, "--json-out", json_out]
    if state_out:
        cmd += ["--state-out", state_out]
    env = dict(os.environ)
    if cache_dir:
        env["ADC_CACHE_DIR"] = cache_dir
    subprocess.run(cmd, check=True, env=env)  # diagnostics -> stdout/stderr ; record -> json_out
...
def orchestrate(args):
    ...
    cache_dir = os.path.join(tmp, "dsl_cache")  # $ADC_CACHE_DIR : vide au COLD, peuple au WARM
    records = []
    ...
    if args.cpp_bin and os.path.exists(args.cpp_bin):
        line = subprocess.check_output(
            [args.cpp_bin, "--n", str(args.n), "--steps", str(args.steps),
             "--warmup", str(args.warmup), "--poisson", args.poisson,
             "--backend", "serial", "--machine", prov_mod.provenance()["machine"]],
        ...
        print("[frontend_compare] front C++ saute (binaire absent : %r)" % args.cpp_bin)

    # 2. Briques (sous-processus frais ; cache DSL non concerne)
    sb = os.path.join(tmp, "state_bricks.npy")
    rec_b = _spawn_worker("bricks", args.n, args.steps, args.warmup, args.poisson,
                          "n/a", os.path.join(tmp, "b.json"), sb)
    ...
    rec_dc = _spawn_worker("dsl", args.n, args.steps, args.warmup, args.poisson,
                           "cold", os.path.join(tmp, "dc.json"), sd, cache_dir=cache_dir)
    rec_dw = _spawn_worker("dsl", args.n, args.steps, args.warmup, args.poisson,
                           "warm", os.path.join(tmp, "dw.json"), None, cache_dir=cache_dir)
    ...
    for r in records:
        ...
        if "nan" in inv:
            ...
            assert inv["rho_min"] > 0, "%s : rho <= 0 (min=%.3e)" % (r["front"], inv["rho_min"])
            assert inv["p_min"] > 0, "%s : p <= 0 (min=%.3e)" % (r["front"], inv["p_min"])
            print("  %-16s mass=%.10e rho_min=%.4e p_min=%.4e cv=%.3f"
                  % (r["front"], inv["mass"], inv["rho_min"], inv["p_min"],
    ...
    for r in records:
        if r["front"] == "cpp" and py_mass is not None:
            dm = abs(r["invariants"]["mass"] - py_mass) / max(abs(py_mass), 1e-30)
            print("  cross-check masse C++ vs Python : derive relative = %.3e" % dm)
            assert dm < 1e-6, "masse C++ vs Python incoherente (derive %.3e)" % dm
    ...
    print("%-18s %12s %12s %12s %12s" % ("front", "cold_user_s", "hot_ms/pas", "advance_ms", "cells/s"))
    cpp_hot = None
    for r in records:
        tag = r["front"] + ("/" + r.get("dsl_cache", "") if r["front"] == "python_dsl" else "")
        hot = r["hot_ms_per_step"]["median"]
        print("%-18s %12.4f %12.4f %12.4f %12.3e"
              % (tag, r.get("total_cold_user_s", 0.0), hot,
                 r.get("advance_ms_per_step", float("nan")), r.get("cells_per_s", 0.0)))
        if r["front"] == "cpp":
            cpp_hot = hot
    if cpp_hot:
        ...
        for r in records:
            if r["front"].startswith("python"):
                tag = r["front"] + ("/" + r.get("dsl_cache", "") if r["front"] == "python_dsl" else "")
                print("  %-18s %.2fx" % (tag, r["hot_ms_per_step"]["median"] / cpp_hot))
    print("\nJSONL ecrit : %s" % jsonl_path)


def main():
    ap = argparse.ArgumentParser(description="Comparaison de fronts C++ / briques / DSL (cas Euler sur)")
    ap.add_argument("--n", type=int, default=128)
    ap.add_argument("--steps", type=int, default=50)
    ap.add_argument("--warmup", type=int, default=5)
    ap.add_argument("--poisson", choices=["off", "on"], default="off")
    ap.add_argument("--cpp-bin", default=None, help="chemin du binaire bench/frontend_cpp (front C++)")
    ap.add_argument("--atol", type=float, default=1e-10, help="tolerance identite numerique briques/DSL")
    ...
    if args.front:  # mode WORKER : un seul front, record -> --json-out
        _bootstrap()
        rec = run_front(args.front, args.n, args.steps, args.warmup, args.poisson,
                        args.dsl_cache, args.state_out)
        with open(args.json_out, "w") as fh:
            json.dump(rec, fh)
        print("[worker %s/%s] cold=%.4fs hot_med=%.4fms"
              % (args.front, args.dsl_cache, rec["total_cold_user_s"],
