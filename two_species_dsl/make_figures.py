#!/usr/bin/env python3
"""Figures de diagnostic du cas two_species_dsl (DSL <-> natif par espece).
...
def density_triptych(
    ...
    axes[2].set_title(
        ...
        + "\n"
        + r"max$|d|_\rho = %.2e$   (etat complet : %.2e)"
        % (rho_max_abs, state_max_abs)
    )
    cb = fig.colorbar(
        im2,
        ax=axes[2],
    ...
    fig.suptitle(
        ...
        "(grille $%d^2$, CFL 0.4, backend %s)"
        % (species_label, NSTEPS, N, backend),
        y=1.02,
    )
    fig.tight_layout()
...
def main() -> None:
    ...
    rho_max_e = density_triptych(
        en[0],
        ed[0],
        "electrons (Euler compressible)",
        max_e,
        os.path.join(FIGDIR, "equivalence_electrons.png"),
        diff_vmax=1e-30,
        backend=backend,
    )
    rho_max_i = density_triptych(
        inn[0],
        idd[0],
        "ions (Euler isotherme)",
        max_i,
        os.path.join(FIGDIR, "equivalence_ions.png"),
        diff_vmax=1e-15,
        backend=backend,
    )
    ...
    prov = {
        ...
        "max_abs_diff_per_var_electrons": {
            ELEC_VARS[k]: float(diff_e[k].max()) for k in range(diff_e.shape[0])
        },
        "max_abs_diff_per_var_ions": {
            ION_VARS[k]: float(diff_i[k].max()) for k in range(diff_i.shape[0])
        },
        "mass_drift_rel_electrons": float(
            case.relative_drift(me_d, float(ne2d.sum()))
        ),
        "mass_drift_rel_ions": float(
            case.relative_drift(mi_d, float(ni2d.sum()))
        ),
    }
    ...
    print(
        "electrons : max|DSL - natif| (etat) = %.3e | sur rho = %.3e (bit-identique etat = %s)"
        % (max_e, rho_max_e, bit_e)
    )
    print(
        "ions      : max|DSL - natif| (etat) = %.3e | sur rho = %.3e (bit-identique etat = %s)"
...
def _compiler_string() -> str:
    """Premiere ligne de `<cxx> --version` du premier compilateur trouve."""
    for cxx in (os.environ.get("CXX"), "c++", "clang++", "g++"):
        if not cxx:
            continue
        try:
            return subprocess.check_output(
                [cxx, "--version"], text=True, stderr=subprocess.DEVNULL
            ).splitlines()[0]
        except Exception:  # noqa: BLE001
            continue
    return "unknown"
