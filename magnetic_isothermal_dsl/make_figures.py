...
def fig_lorentz_oracle(backend, sim_b0, sim_0, state0):
    """dR = rhs(B0) - rhs(0), confronte a (B0 my0, -B0 mx0) numpy. Renvoie les nombres mesures."""
    mx0, my0 = state0[1], state0[2]
    dR = np.array(sim_b0.eval_rhs("plasma")) - np.array(sim_0.eval_rhs("plasma"))
    lor_x = R.B0 * my0            # +B_z my sur la qte de mvt x (ici my0 = 0 -> identiquement nul)
    ...
    for ax, err, title in (
        ...
        ax.set_title(title + "\nmax|.| = %.1e (echelle +/- eps_mach)"
                     % float(np.max(np.abs(err))), fontsize=10)
        ax.set_xticks([]); ax.set_yticks([])
        cb = fig.colorbar(im, ax=ax, fraction=0.046, pad=0.04)
        cb.set_ticks([-eps, 0, eps]); cb.ax.tick_params(labelsize=7)
    fig.suptitle("Oracle Lorentz [backend %r] : residu DSL - forme analytique numpy. "
                 "Echelle +/- eps_machine : tout pixel non nul saturerait ; ici tout est 0." % backend,
                 fontsize=11)
...
def fig_cyclotron_trajectory(backend, sim_b0):
    ...
    ts.append(sim_b0.time()); mxm.append(float(st[1].mean())); mym.append(float(st[2].mean()))
    for _ in range(NSTEPS):
        sim_b0.step_cfl(CFL)
        st = np.array(sim_b0.get_state("plasma"))
        ts.append(sim_b0.time()); mxm.append(float(st[1].mean())); mym.append(float(st[2].mean()))
    ts = np.array(ts); mxm = np.array(mxm); mym = np.array(mym)
    ...
    axL.plot(cx, cy, "-", color="0.6", lw=1.4,
             label=r"cercle cyclotron $r_0(\cos\omega_c t,\sin\omega_c t)$")
    axL.plot(mxm, mym, "o-", color="C0", ms=3.5, lw=1.0, label="mesure DSL (moyenne par pas)")
    axL.plot([mxm[0]], [mym[0]], "s", color="C2", ms=9, label="depart $(u_0, 0)$")
    axL.plot([mxm[-1]], [mym[-1]], "*", color="C3", ms=14, label="arrivee")
    axL.axhline(0, color="0.85", lw=0.7); axL.axvline(0, color="0.85", lw=0.7)
    axL.set_xlabel(r"$\langle m_x\rangle$"); axL.set_ylabel(r"$\langle m_y\rangle$")
    axL.set_aspect("equal"); axL.legend(fontsize=8, loc="lower left")
    ang = np.degrees(np.arctan2(mym[-1], mxm[-1]))
    axL.set_title(r"$\omega_c = qB_z = %.1f$ ; angle final mesure $%.2f^\circ$"
                  r" (predit $\omega_c t = %.2f^\circ$)"
                  % (omega_c, ang, np.degrees(omega_c * ts[-1])), fontsize=10)

    mag = np.hypot(mxm, mym)
    axR.plot(ts, mag, "o-", color="C0", ms=3.5, lw=1.0)
    axR.axhline(r0, color="0.6", lw=1.2, ls="--", label=r"$r_0 = |\langle m\rangle|_{t=0}$")
    axR.set_xlabel("t"); axR.set_ylabel(r"$|\langle m\rangle|$")
    axR.set_title(r"module conserve : derive relative $%.1e$"
                  % ((mag[-1] - r0) / r0), fontsize=10)
    ...
    return {
        ...
        "t_final": float(ts[-1]),
        "mx_mean_init": float(mxm[0]), "my_mean_init": float(mym[0]),
        "mx_mean_final": float(mxm[-1]), "my_mean_final": float(mym[-1]),
        "angle_final_deg": float(ang),
        "angle_predicted_deg": float(np.degrees(omega_c * ts[-1])),
        "mag_init": float(r0), "mag_final": float(mag[-1]),
...
def main():
    ...
    prov = {
        ...
        "backend": "DSL %r (production non lie : ABI en-tetes du module pre-construit)" % backend,
        "backends_linked": backends,
        "resolution": "%dx%d" % (N, N),
        "nsteps": NSTEPS,
        "cfl": CFL,
        "cs2": R.CS2, "charge_q": R.Q, "B_z": R.B0,
        "python": "3.12.2",
