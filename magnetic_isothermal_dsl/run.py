...
def magnetic_isothermal_model():
    ...
    m.flux(x=[mx, mx * u + cs2 * rho, mx * v],
           y=[my, my * u, my * v + cs2 * rho])
...
def _build_sim(n, compiled, state0, bz_value):
    """Construit un System periodique, installe le bloc DSL @p compiled, fixe l'etat, peuple le
    champ B_z constant a @p bz_value, puis resout les champs (phi / grad / B_z dans le canal aux).
    Le meme schema (minmod + Rusanov, explicite) que les autres demonstrateurs DSL."""
    sim = adc.System(n=n, L=1.0, periodic=True)
    sim.add_equation("plasma", model=compiled,
...
def bind_backends(n, state0, bz_value):
    ...
    for cand in ("production", "aot"):
        ...
        bound[cand] = sim
    if not bound:
        raise RuntimeError("aucun backend DSL n'a pu etre lie pour le modele magnetise")
    return bound
...
def main():
    ...
    print("=== magnetic_isothermal_dsl : fluide isotherme magnetise ecrit en formules ===")
    print("grille %d x %d, %d pas, CFL = 0.4 ; cs2 = %.1f, q = %.0f, B_z = %.1f"
    ...
    if len(backends) >= 2:
        ...
        for b in backends[1:]:
            ...
            print("eval_rhs %r vs %r : dmax = %.3e (bit-identique = %s)"
    ...
    lor_contrib = 0.0
    for b in backends:
        dR = np.array(bound[b].eval_rhs("plasma")) - np.array(bound0[b].eval_rhs("plasma"))
        ...
        print("oracle Lorentz [%r] : err_x = %.3e, err_y = %.3e, max|dR| = %.3e"
              % (b, err_x, err_y, mag))
    ...
    print("apres %d pas (backend %r) : t = %.6f, derive de masse = %.3e"
          % (n_steps, backends[0], sim.time(), drift))
    print("qte de mvt transverse moyenne : initiale %.3e -> finale %.3e (rotation de Lorentz)"
          % (my_mean0, my_mean))
    ...
    assert abs(my_mean) > 1e-6, (
        "la quantite de mouvement transverse est restee nulle : Lorentz n'a pas devie l'ecoulement")

    print("OK magnetic_isothermal_dsl (Lorentz exerce, B_z = %.1f pilote depuis Python, backends %s)"
