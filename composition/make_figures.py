...
def partie_A_fields():
    ...
    return {
        "rho_e_ci": rho_e_ci, "rho_e_fin": rho_e_fin, "rho_i_fin": rho_i_fin,
        "phi_fin": phi_fin, "phi_amp0": phi_amp0,
...
def fig_density_maps(fa):
    ...
    panel(axes[0], fa["rho_e_ci"],
          "electrons : densite CI\n1 + 0.02 cos(2pi x)")
    panel(axes[1], fa["rho_e_fin"],
          "electrons : finale (t=%.3f)\nEuler / HLLC / IMEX(10)" % (DT_A * NSTEPS_A))
    panel(axes[2], fa["rho_i_fin"],
...
def fig_determinism(da_B, db_B, da_D, db_D):
    diff_B = np.abs(da_B - db_B)
    diff_D = np.abs(da_D - db_D)
    fig, axes = plt.subplots(1, 3, figsize=(13.5, 4.0))

    # (B) heatmap de l'ecart : doit etre identiquement noir.
    imB = axes[0].imshow(diff_B, origin="lower", cmap="inferno",
                         extent=[0, L, 0, L], vmin=0.0, vmax=1e-15)
    axes[0].set_title("(B) |rho_compo1 - rho_compo2|\ndeux compositions C++ (diocotron)",
                      fontsize=10)
    axes[0].set_xticks([0, L]); axes[0].set_yticks([0, L])
    ...
    axes[2].hist(allres, bins=np.linspace(-5e-16, 5e-16, 11),
                 color="#444", edgecolor="k")
    axes[2].axvline(0.0, color="crimson", lw=1.2, ls="--")
    axes[2].set_title("residu |a - b| (B et D)\ntoute la masse a exactement 0",
                      fontsize=10)
    ...
    fig.savefig(out, dpi=130); plt.close(fig)
    return out, float(diff_B.max()), float(diff_D.max()), \
        bool(np.array_equal(da_B, db_B)), bool(np.array_equal(da_D, db_D))
...
def main():
    ...
    prov = {
        ...
        "resolution": {"partie_A": f"{N_A}x{N_A}", "parties_BD": f"{N_BD}x{N_BD}"},
        "periodic": True,
        "schemes": {
    ...
    print("  ", out_maps)
    print("  ", out_det)
    print(f"  Partie A : |phi|_max = {fa['phi_amp0']:.6e}, evolution electrons = {fa['evol_e']:.3e}")
    print(f"  Partie B : compose bit diff = {max_B:.3e}, array_equal = {eq_B}")
    print(f"  Partie D : pystep bit diff = {max_D:.3e}, array_equal = {eq_D}, mass drift = {dm_D:.3e}")
