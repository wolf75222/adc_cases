...
def sha(path: str) -> str:
    """SHA du HEAD git du depot a `path` (ou "unknown" si indisponible)."""
    try:
...
def fig_bars(csv_path: str, rows: list) -> str:
    ...
    ax.set_title(
        "dt stable par methode (omega_c = %g, t_end = 1.0, n=16, cs2=1e-4)"
        % OMEGA_C_REF,
        fontsize=10,
    )
    de = dts[0]
...
def fig_vs_omega(meas: dict) -> str:
    ...
    if w0 is not None:
        ...
        a1.loglog(
            wcs,
            [d0 * w0 / w for w in wcs],
            ":",
            color="#c44e52",
            lw=1,
            label="pente -1 (1/omega_c)",
        )
    ...
    a2.set_title(
        "produit dt*omega_c : explicite ~ O(1), Schur non borne", fontsize=10
    )
    a2.legend(fontsize=8)
    a2.grid(True, which="both", alpha=0.3)

    fig.suptitle(
        "Mesure fraiche ciblee (n=16, cs2=1e-4, t_end=%.2f, alpha=1)"
        % meas["t_end"],
        fontsize=10,
    )
