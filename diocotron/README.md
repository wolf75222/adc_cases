...
| `models.diocotron(...)` -> `ExBVelocity` / `BackgroundDensity` (`include/adc/physics/{hyperbolic,elliptic}.hpp`) | brique C++ **fige** la physique | la convention exacte du flux $n v(dir)$, de la valeur propre $v(dir)$, du RHS $\alpha(n-n_{i0})$ |
