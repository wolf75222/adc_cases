...
def make_system(n: int, l: int, limiter: str, delta: float) -> adc.System:
    ...
    """
    time = (
        adc.Explicit(method="ssprk3") if limiter == "weno5" else adc.Explicit()
    )
    sim = adc.System(n=n, L=dioc.L, periodic=False)
    sim.add_block(
        "ne",
        model=models.diocotron(B0=dioc.B0, alpha=dioc.ALPHA, n_i0=0.0),
        spatial=adc.Spatial(limiter=limiter, flux="rusanov"),
        time=time,
    )
    sim.set_poisson(
        rhs="charge_density",
...
def measure_growth(
    ...
    while True:
        ...
        if not np.isfinite(phi).all():
            print(
                f"  [n={n} {limiter} l={l}] NaN au pas {step} -> arret",
                file=sys.stderr,
            )
            break
        ts.append(sim.time())
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--modes", default="3,4,5", help="modes azimutaux l (defaut 3,4,5)."
    )
    p.add_argument(
        "--ns",
        default="128,192,256",
        help="resolutions n (defaut 128,192,256).",
    )
    p.add_argument(
        "--orders",
        default="minmod,vanleer,weno5",
    ...
    p.add_argument(
        ...
        type=float,
        default=T_END,
        help="temps physique final commun.",
    )
    p.add_argument(
        "--quick",
        action="store_true",
...
def main() -> None:
    ...
    for o in orders:
        if o not in ORDER_LABEL:
            raise SystemExit(
                f"ordre '{o}' non atteignable via add_block ; choisir parmi "
    ...
    print("=" * 78)
    print(
        "Balayage diocotron : ordre x resolution x mode (mesure, PR-0 Hoffart)"
    )
    print(
    ...
    print("=" * 78)
    print(
        f"{'n':>5} {'ordre':>11} {'l':>3} {'gamma_num':>10} {'gamma_ana':>10} "
    ...
    for o in orders:
        for n in ns_list:
            for l in modes:
                (g, win, tf, nstep, fit_i0, fit_i1, fit_t0, fit_t1) = (
                    measure_growth(l, n, o, t_end=args.t_end)
                )
                ga = gamma_ana[l]
                err = 100.0 * (g - ga) / ga
                print(
                    f"{n:>5} {ORDER_LABEL[o]:>11} {l:>3} {g:>10.4f} {ga:>10.4f} "
    ...
    with open(csv_path, "w") as f:
        ...
        f.write(
            "n,order,order_label,l,gamma_num,gamma_ana,err_pct,t_final,nsteps,"
            "fit_i0,fit_i1,fit_t0,fit_t1\n"
        )
        for r in rows:
            f.write(
                f"{r[0]},{r[1]},{r[2]},{r[3]},{r[4]:.6f},{r[5]:.6f},{r[6]:.3f},"
