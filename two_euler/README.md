...
Poisson de systeme ($f=\sum_b q_b n_b$) est zero. `set_poisson()` est appele tout de meme car
...
| `models.euler(GAMMA)` -> `CompressibleFlux` / `NoSource` / `ChargeDensity(0)` (`include/adc/physics/euler.hpp`, `.../source.hpp`) | la brique C++ fige la physique | la convention exacte du flux $F(U)$, de la vitesse d'onde $|v_n|+c$, du second membre nul |
| `assemble_rhs<Limiter,Flux>` (HLLC) + Poisson de systeme (`solve_fields`, $f=0$) | noyau par cellule | le calcul par cellule, sans callback Python dans le hot path |
...
seulement par `set_state` (section 5).
...
`sigma2=0.02` n'est pas passe par `run.py` : c'est la valeur par defaut de `euler_pressure_blob`.
...
- Pas une physique de plasma, pas une reproduction. Les blocs "electrons"/"ions" ne sont pas
  couples : source nulle, Poisson a second membre nul ($q=0$). Aucun champ ne les relie. Un
...
`cpython-3XY`), et le paquet `adc_cases` sur le `PYTHONPATH`. Aucun compilateur C++ n'est requis a
l'execution : le modele est compose de briques natives deja compilees dans `adc`.

Sortie capturee (run du 2026-06-08, deterministe sur executions consecutives) :
