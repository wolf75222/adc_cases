...
analytique. Notre `DiocotronSolver` EST ce modele reduit de derive E x B (la limite visee) :
on reproduit donc directement la figure-cle du papier (gamma numerique vs analytique vs mode l).

Ce qui est produit (dans python/figures_2510_11808/) :
...
Usage : PYTHONPATH=<dossier du module adc> python3 repro_paper_2510_11808.py
"""
import math
import os
import sys
...
OUT = os.path.join(os.path.dirname(os.path.abspath(__file__)), "figures_2510_11808")
...
def measure_growth(l, n=192, delta=0.01, cfl=0.4, nsteps=900, sample_every=4):
    """Lance le diocotron mode l et mesure gamma_norm = gamma * 2 pi / rhobar."""
    cfg = adc.DiocotronConfig()
    cfg.n = n; cfg.L = L; cfg.B0 = B0; cfg.alpha = ALPHA
    cfg.ic = adc.DiocotronIC.Ring
    cfg.ring_r0 = R0; cfg.ring_r1 = R1; cfg.wall_radius = RWALL
    cfg.ring_mode = l; cfg.ring_delta = delta; cfg.ring_floor = 1e-3
    cfg.poisson_per_stage = True
...
def run_evolution(l=4, n=192, delta=0.1, cfl=0.4, nframes=60, steps_per_frame=12):
    ...
    for f in range(nframes):
        ...
        if not np.isfinite(d).all():
            break
        frames.append(d.copy()); times.append(s.time())
...
def main():
    ...
    print("\n[numerique] mesure du taux par DiocotronSolver (n=192, delta=0.01) :")
    runs = {}
    for l in MODES:
