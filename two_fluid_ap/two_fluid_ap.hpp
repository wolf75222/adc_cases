...
// Solveur deux-fluides isotherme 2D asymptotic-preserving (AP).
//
// Cet integrateur a quitte le coeur adc_cpp : c'est un SCENARIO, pas une brique generique
// composable bloc a bloc. La stabilisation AP couple la raideur (frequence plasma) au pas
// de temps DANS l'elliptique (Poisson reformule), ce que la composition adc.System ne sait
...
// Pas deux-fluides isotherme 2D asymptotic-preserving, PORTABLE GPU. Tous les kernels
// passent par for_each_cell avec des lambdas ADC_HD (device-callables) : pas de lambda
// imbriquee, et abs/max via ternaires (std::fabs/std::fmax ne sont pas device-safe ;
// std::sqrt l'est et n'est utilise qu'en hote). Le champ est delegue a un EllipticSolver template (PoissonFFT
